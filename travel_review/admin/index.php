...
function viago_admin_log_moderation(PDO $pdo, string $targetType, int $targetId, string $action, ?string $reason = null, ?int $aiScore = null, ?string $aiService = null, ?int $adminId = null): void
{
    if ($targetId <= 0) {
        return;
    }
    ...
    try {
        $adminId = $adminId ?? (int)($_SESSION['user_id'] ?? 0);
        $reason = $reason !== null && $reason !== '' ? mb_substr($reason, 0, 255, 'UTF-8') : null;
        $aiService = $aiService !== null && $aiService !== '' ? mb_substr($aiService, 0, 50, 'UTF-8') : null;

        $stmt = $pdo->prepare("
            INSERT INTO moderation_logs
                (target_type, target_id, action, reason, ai_score, ai_service, admin_id)
            VALUES
                (?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
        ...
        // migration 적용 전에도 관리자 페이지가 깨지지 않도록 로그 실패는 서버 로그에만 남깁니다.
        error_log('[Moderation Log Error] ' . $e->getMessage());
    }
}

function viago_admin_get_review_moderation_summary(PDO $pdo, int $reviewId): array
{
    $summary = [
        'ad_score' => 0,
        'text_reason' => null,
    ...
    try {
        $stmt = $pdo->prepare("SELECT ad_score, moderation_reason FROM reviews WHERE id=?");
        $stmt->execute([$reviewId]);
        $review = $stmt->fetch();
        if ($review) {
            $summary['ad_score'] = (int)($review['ad_score'] ?? 0);
            $summary['text_reason'] = $review['moderation_reason'] ?? null;
        }

        $stmt = $pdo->prepare("
            SELECT
                COALESCE(MAX(moderation_score), 0) AS image_score,
                GROUP_CONCAT(DISTINCT moderation_reason SEPARATOR ', ') AS image_reason
            FROM review_media
            WHERE review_id=?
        ...
        $media = $stmt->fetch();
        if ($media) {
            $summary['image_score'] = (int)($media['image_score'] ?? 0);
            $summary['image_reason'] = $media['image_reason'] ?? null;
        }

        if ($summary['image_score'] > $summary['ad_score']) {
            $summary['score'] = $summary['image_score'];
            $summary['service'] = 'rekognition';
            $summary['reason'] = $summary['image_reason'] ?: $summary['text_reason'];
        } else {
            $summary['score'] = $summary['ad_score'];
            $summary['service'] = $summary['ad_score'] > 0 ? 'comprehend' : ($summary['image_score'] > 0 ? 'rekognition' : null);
            $summary['reason'] = $summary['text_reason'] ?: $summary['image_reason'];
        }
    } catch (Throwable $e) {
        error_log('[Moderation Summary Error] ' . $e->getMessage());
    }

    return $summary;
}

function viago_admin_delete_comment_with_reports(PDO $pdo, int $commentId): void
{
    if ($commentId <= 0) {
        return;
    }

    viago_admin_log_moderation($pdo, 'comment', $commentId, 'deleted', '관리자 또는 신고 처리에 의한 댓글 삭제', null, null);

    $pdo->prepare("DELETE FROM reports WHERE target_type='comment' AND target_id=?")
        ->execute([$commentId]);
...
function viago_admin_delete_review_with_assets(PDO $pdo, int $reviewId): void
{
    if ($reviewId <= 0) {
        return;
    }

    $summary = viago_admin_get_review_moderation_summary($pdo, $reviewId);
    viago_admin_log_moderation(
        $pdo,
        'review',
        $reviewId,
        'deleted',
        $summary['reason'] ?: '관리자 검토에 의한 리뷰 삭제',
        $summary['score'] > 0 ? $summary['score'] : null,
        $summary['service']
    );

    // 리뷰 삭제 전에 연결된 S3/CloudFront 미디어 경로를 먼저 조회해야 합니다.
    // reviews 삭제 후에는 review_media가 FK CASCADE로 사라져 S3 객체 key를 알 수 없습니다.
    $stmt = $pdo->prepare("SELECT id, file_path, moderation_score, moderation_reason FROM review_media WHERE review_id=?");
    $stmt->execute([$reviewId]);
    ...
    foreach ($mediaFiles as $media) {
        if (!empty($media['file_path'])) {
            ...
            viago_admin_log_moderation(
                ...
                (int)$media['id'],
                'deleted',
                $media['moderation_reason'] ?: '리뷰 삭제에 따른 S3 미디어 삭제',
                isset($media['moderation_score']) ? (int)$media['moderation_score'] : null,
                'rekognition'
            );
        }
    }

    // reports는 target_id에 FK가 없으므로 리뷰 삭제와 별도로 정리합니다.
    $pdo->prepare("DELETE FROM reports WHERE target_type='review' AND target_id=?")
        ->execute([$reviewId]);

    // comments, likes, review_media는 스키마의 ON DELETE CASCADE로 함께 삭제됩니다.
    $pdo->prepare("DELETE FROM reviews WHERE id=?")
        ->execute([$reviewId]);
}

function viago_admin_delete_user_with_assets(PDO $pdo, int $userId): void
{
    if ($userId <= 0) {
        return;
    }

    // 사용자의 프로필 이미지도 S3에 있으면 함께 삭제합니다.
    $stmt = $pdo->prepare("SELECT profile_image FROM users WHERE id=?");
    $stmt->execute([$userId]);
    $user = $stmt->fetch();

    if ($user && !empty($user['profile_image'])) {
        viago_delete_media_file($user['profile_image']);
    }

    // 사용자가 작성한 리뷰를 먼저 삭제하여 S3 미디어를 정리합니다.
    $stmt = $pdo->prepare("SELECT id FROM reviews WHERE user_id=?");
    $stmt->execute([$userId]);
    $reviewIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

    foreach ($reviewIds as $reviewId) {
        viago_admin_delete_review_with_assets($pdo, (int)$reviewId);
    }

    // 사용자가 작성한 댓글을 삭제하기 전에 해당 댓글 대상 신고도 정리합니다.
    $stmt = $pdo->prepare("SELECT id FROM comments WHERE user_id=?");
    $stmt->execute([$userId]);
    $commentIds = $stmt->fetchAll(PDO::FETCH_COLUMN);

    foreach ($commentIds as $commentId) {
        viago_admin_delete_comment_with_reports($pdo, (int)$commentId);
    }

    viago_admin_log_moderation($pdo, 'user', $userId, 'deleted', '관리자에 의한 회원 삭제', null, null);

    // user_id FK로 연결된 reports, likes 등은 ON DELETE CASCADE로 정리됩니다.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'delete_user':
            $uid = (int)$_POST['user_id'];
            if ($uid !== (int)$_SESSION['user_id']) {
                viago_admin_delete_user_with_assets($pdo, $uid);
            }
            break;
        ...
        case 'approve_review':
            ...
            viago_admin_log_moderation(
                ...
                $reviewId,
                'approved',
                $summary['reason'] ?: '관리자 검토 후 게시글 승인',
                $summary['score'] > 0 ? $summary['score'] : null,
                $summary['service']
            );
        ...
        case 'delete_review':
            viago_admin_delete_review_with_assets($pdo, (int)$_POST['review_id']);
            break;

        case 'dismiss_report':
            $reportId = (int)$_POST['report_id'];
            viago_admin_log_moderation($pdo, 'report', $reportId, 'dismissed', '관리자 검토 후 신고 무시', null, null);
        ...
        case 'delete_report_target':
            ...
            $reportId = (int)$_POST['report_id'];
            viago_admin_log_moderation($pdo, 'report', $reportId, 'deleted', '신고 대상 삭제 처리', null, null);
            if ($type === 'review') {
                viago_admin_delete_review_with_assets($pdo, $targetId);
            } elseif ($type === 'comment') {
                viago_admin_delete_comment_with_reports($pdo, $targetId);
            }
...
try {
    $totalModerationLogs = (int)$pdo->query("SELECT COUNT(*) FROM moderation_logs")->fetchColumn();
} catch (Throwable $e) {
    $totalModerationLogs = 0;
}
...
$moderationLogs = [];
try {
    $moderationLogs = $pdo->query("
        SELECT moderation_logs.*, users.username AS admin_name
        FROM moderation_logs
        LEFT JOIN users ON users.id = moderation_logs.admin_id
    ...
    <ul class="nav admin-nav-tabs" id="adminTabs" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tab-logs" type="button">
                <i class="bi bi-clipboard-check"></i> 처리 로그
                <?php if ($totalModerationLogs > 0): ?>
                    <span class="tab-badge blue"><?= $totalModerationLogs ?></span>
                <?php endif; ?>
            </button>
        </li>
...
<div class="tab-content admin-tab-content">
    ...
    <div class="tab-pane fade" id="tab-logs" role="tabpanel">
        <div class="admin-card">
            <div class="admin-card-header">
                <h2><i class="bi bi-clipboard-check" style="color:#0d6efd;"></i> 관리자 처리 로그</h2>
                <span style="font-size:0.82rem;color:#6c8ea4;">최근 <?= count($moderationLogs) ?>건</span>
            </div>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="muted"><?= date('m-d H:i', strtotime($log['created_at'])) ?></td>
                        <td>
                            <span class="badge-normal"><?= htmlspecialchars($log['target_type']) ?></span>
                            <span class="muted">#<?= (int)$log['target_id'] ?></span>
                        </td>
                        <td>
                            <span class="badge-<?= $log['action'] === 'approved' ? 'normal' : ($log['action'] === 'dismissed' ? 'reported' : 'flagged') ?>">
                                <?= ['approved'=>'승인','deleted'=>'삭제','blocked'=>'차단','dismissed'=>'무시','role_changed'=>'권한변경'][$log['action']] ?? htmlspecialchars($log['action']) ?>
                            </span>
                        </td>
                        <td class="muted">
                            <?= htmlspecialchars($log['ai_service'] ?: '—') ?>
                            <?php if ($log['ai_score'] !== null): ?>
                                / <?= (int)$log['ai_score'] ?>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="muted"><?= htmlspecialchars($log['admin_name'] ?: 'unknown') ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
